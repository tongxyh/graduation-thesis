# 一、概述
图像存储在三维矩阵中，保存着图片在二维空间中各个像素位置的色彩、亮度等信息。常见的形式包括RGB(红绿蓝)、YUV(Y是亮度通道、UV是两个色度通道)等，矩阵中的每个位置都由多个比特的整形数据表示，因此一张完整的未压缩图像数据量较大并且包含较多的冗余。互联网时代图片和视频占据了大量的存储空间，因此高效的图像压缩技术对于提高存储效率有非常重要的意义。
图像压缩主要包括 *有损压缩* 和 *无损压缩* 两种。  
无损压缩顾名思义，压缩后的数据可以完全解压为原始数据，不会造成任何的损失。其主要代表有PNG等。  
有损压缩在压缩过程中会带来一定的信号损失，不能完全还原出原始数据。其主要代表有JPEG、JPEG2000等。
...
### 2.2 JPEG2000
JPEG 2000 [8] 是基于小波变换的图像压缩标准，由Joint Photographic Experts Group组织创建和维护。JPEG 2000通常被认为是替代JPEG的下一代图像压缩标准。
#### 优势：
JPEG2000的压缩比更高，而且不会产生原先的基于离散余弦变换的JPEG标准产生的块状模糊瑕疵。JPEG2000同时支持有损压缩和无损压缩。另外，JPEG2000也支持更复杂的渐进式显示和下载。JPEG2000的失真主要是模糊失真。模糊失真产生的主要原因是在编码过程中高频量一定程度的衰减。传统的JPEG压缩也存在模糊失真的问题。就图像整体压缩性能来说，在低压缩比情形下（比如压缩比小于10：1），传统的JPEG图像质量有可能要比JPEG2000要好。JPEG2000在压缩比比较高的情形下，优势才开始明显。整体来说，和传统的JPEG相比，JPEG2000仍然有很大的技术优势，通常压缩性能大概可以提高20%以上。一般在压缩比达到100：1的情形下，采用JPEG压缩的图像已经严重失真并开始难以识别了，但JPEG2000的图像仍可识别。

### 2.3 [BPG](https://bellard.org/bpg/)
BPG (Better Portable Graphics) 是一种新的图像格式.
它的目标是代替传统的JPEG格式。  
它的主要特点在于：  
（1）基于HEVC视频压缩标准.  
（2）相同恢复质量下，有更高的压缩率  
（3）支持无损压缩  
由于基于HEVC，它是目前性能最强的图像压缩算法。

# 三、基于学习的方法   
随着计算能力的发展，深度学习已经在计算机视觉等领域展现出强大的力量，使得通过对大量自然图片进行学习，获得其中的特征表达成为可能。近几年一些学者已经在这方面做出了非常优秀的成果。包括Google [@cite]、WaveOne [@cite]、ETH [@cite]等。

## 3.1 对传统压缩方法的增强
深度学习在SR、Deblocking等方面已经出现了许多成功的应用，因此在保持原始图像压缩框架不变的情况下，深度学习可以作为对传统编码进行直接后处理的一种有效工具。  
其基本思路就是对有损压缩(JPEG,JPEG2000等)的输出进行图像恢复和怎强，通过学习消除有损压缩产生的噪声和损失等。这种方法应用速度快。缺点在于需要对于不同的压缩、不同的QP都需要进行单独训练，通用性不强。
...
对整个网络结构进行了如下改进
#### (1) 引入量化层
根据 [xxx] 中，由于量化操作的导数几乎处处为0。所以不能直接加入到网路中去，需要对其做一些近似操作。具体的实现为：
正向时，直接计算：  
y = round(Bits * sigmoid(x))  
反向时则跳过该层，将梯度直接传递给下一层。  
...
## 4.2 训练
### 1. 数据集准备  
数据集包括 [kodak PhotoCD dataset](http://r0k.us/graphics/kodak), 以及 [CVPR 2018 CLIC WORKSHOP](http://www.compression.cc) 提供的数据集。将收集的图片裁剪为128x128的块供训练使用,并且控制块与块之间的相似度。（数据集多为2K图片，常常会出现大块的纹理相似的部分，不做剔除可能导致过拟合）
...
最终得到随着训练进行的RD优化曲线，如下图所示：
<center> <img src='pic/rd.png'/> </center>

## 4.3 实验与结果分析
在无损编码部分，目前使用的是PAQ。在 Kodak PhotoCD数据集 和 clic给出的测试集上进行了测试了
### 1. 性能
图1
图2
图3
...
### 2. CLIC2018
目前的算法在CLIC 2018中进行了验证  
图1

### 3. 计算复杂度与计算资源占用
实验所用设备的具体配置为 Inter Xeon E5-1680 v4 cpu(3.40GHz,8cores,16processors) + NVIDIA P5000 GPU + 64G内存(32G ram + 32G swap)。在此平台上测试的结果如下：
#### 编码复杂度
    对于分辨率720x1248的图片,编码速度为6.6fps左右  
#### 解码复杂度  
    待测
